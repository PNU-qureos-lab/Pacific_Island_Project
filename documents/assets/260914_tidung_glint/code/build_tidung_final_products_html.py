...
def fixed_water_limits(decisions: list[dict[str, str]]) -> tuple[np.ndarray, np.ndarray]:
    """One reproducible RGB display range shared by every scene and stage."""
    ...
    for decision in decisions:
        ...
        for channel, band in enumerate(("B04", "B03", "B02")):
            paths = [
                JOINT / "02_Multiband_Inputs" / scene / f"{band}_BOA_20m.tif",
                JOINT / "03_Glint_ACOLITE_Cycle0" / scene / f"Cycle0_{band}.tif",
            ]
            if summary["final_qc"]["retained_product"] != "Cycle0":
                paths.append(ROOT / "FINAL_PRODUCTS" / scene / "01_Final_BOA_Reflectance" / f"{scene}_{band}_final_BOA_reflectance.tif")
            for path in paths:
                a, _ = read_band(path)
                values = a[::20, ::20]
                sampled[channel].append(values[np.isfinite(values)])
    # The low tail keeps dark water visible; the 95th percentile prevents bright
    # island/cloud pixels from controlling the water display.
    channel_high = [float(np.percentile(np.concatenate(v), 95)) for v in sampled]
    # A common physical range preserves the B04:B03:B02 balance. Independent
    # channel stretching would manufacture hue changes (notably a pink cast).
    common_high = max(channel_high)
    return np.zeros(3), np.full(3, common_high)
...
def main() -> None:
    ...
    water_low, water_high = fixed_water_limits(decisions)
    scene_data = []
    for decision in decisions:
        scene = decision["scene"]
        summary = json.loads((SOURCE / "scenes" / scene / "experiment_summary.json").read_text(encoding="utf-8"))
        ...
        for label, arr in zip(("before", "cycle0", "final"), rgb_sets):
            ...
            images[label] = rel(p)
            wp = ASSETS / scene / f"{label}_fixed_water_stretch.png"
            save_rgb(wp, arr, water_low, water_high)
            water_images[label] = rel(wp)
        ...
        scene_data.append({
            ...
            "images": images, "waterImages": water_images, "bounds": lonlat_bounds,
            "figures": figures,
            "evidence": {
            ...
            "qc": {
                ...
                "RrsB01": float(band_qc["B01"]["above_surface_Rrs_median_sr-1"]),
    ...
    payload = json.dumps({"bands": list(BANDS), "wavelengths": list(WAVE), "scenes": scene_data, "archive": ledger,
                          "fixedRgb": {"low": water_low.tolist(), "high": water_high.tolist(), "gamma": 0.85}}, separators=(",", ":"))
    ...
    html = html.replace(
        'All three panels use the same water-focused RGB stretch.',
        'All three panels use one shared water-focused RGB display stretch: the 1st and 99.5th percentiles of B04/B03/B02 across the three processing stages, followed by display gamma 0.85. This changes only the displayed colour, never the BOA or Rrs values.',
    )
    ...
    html = html.replace('Glint estimate used to classify each ROI', 'Thirty selected deep-water ROIs ranked by glint indicator')
    ...
    html = html.replace(
        'Across all eligible offshore ROI centres in the scene, the 33⅓% and 66⅔% quantiles of the 7 × 7 mean ρg,ref define LOW, MEDIUM and HIGH candidate pools. Ten spatially separated ROI centres are then selected from each pool. The chart below sorts only the 30 selected ROIs for display; it did not create the boundaries.',
        '<b>How the ranges are chosen:</b> all eligible 7 × 7 offshore-water centres are ranked using their mean selected SWIR glint indicator. The scene-specific 33⅓ percentile is the LOW/MEDIUM boundary and the 66⅔ percentile is the MEDIUM/HIGH boundary. LOW is at or below the first boundary; MEDIUM is above the first and at or below the second; HIGH is above the second. Ten spatially separated ROIs are then selected from each pool. The graph shows those 30 selected ROIs; it does not create the boundaries.<div class="formula">Eligible offshore centres → calculate mean glint indicator → sort → divide at 33⅓% and 66⅔% → select 10 LOW + 10 MEDIUM + 10 HIGH</div>',
    )
    # The earlier comprehensive renderer includes the large ROI map, ranking
    # graph and all downstream evidence. Keep the later legacy renderer from
    # overriding it in JavaScript.
    html = html.replace(
        'function orderedEvidence(s){const e=s.evidence,med=g=>median(',
        'function orderedEvidenceLegacy(s){const e=s.evidence,med=g=>median(',
        1,
    )
    html = html.replace(
        "${metric('LOW ρg,ref median',med('LOW').toFixed(6))}${metric('MEDIUM ρg,ref median',med('MEDIUM').toFixed(6))}${metric('HIGH ρg,ref median',med('HIGH').toFixed(6))}",
        "${metric('LOW selected range',Math.min(...s.roi.filter(r=>r.group==='LOW').map(r=>r.rhog)).toFixed(6)+' to '+Math.max(...s.roi.filter(r=>r.group==='LOW').map(r=>r.rhog)).toFixed(6))}${metric('MEDIUM selected range',Math.min(...s.roi.filter(r=>r.group==='MEDIUM').map(r=>r.rhog)).toFixed(6)+' to '+Math.max(...s.roi.filter(r=>r.group==='MEDIUM').map(r=>r.rhog)).toFixed(6))}${metric('HIGH selected range',Math.min(...s.roi.filter(r=>r.group==='HIGH').map(r=>r.rhog)).toFixed(6)+' to '+Math.max(...s.roi.filter(r=>r.group==='HIGH').map(r=>r.rhog)).toFixed(6))}",
    )
    html = re.sub(
    ...
    html = html.replace('</svg></div><div class="card note"><b>Interpretation:</b> after correction,', '</svg><details><summary><b>Final approximate Rrs group-median values</b></summary><div id="rrsTable"></div></details></div><div class="card note"><b>Interpretation:</b> choose Final approximate Rrs to plot all 30 ROI spectra in sr⁻¹. Rrs is calculated as final BOA / π. It passed numerical conversion checks but is not field-validated water-leaving reflectance. After correction,')
    ...
    # This file is the complete deep-water report. Shallow-water validation is
    # exposed separately as top-level Tab 4 in the primary report.
...
'''
    ...
    merged = merged.replace(
        '</nav>',
        '<button class="tab-button" data-tab="deep-results">3 · Deep-water results</button><button class="tab-button" data-tab="shallow-results">4 · Shallow-water results</button><button class="tab-button" data-tab="code">5 · Code and outputs</button></nav>',
        1,
    )
...
'''
    result_panel = '''
<section id="deep-results" class="panel">
  <div class="section-head">
    <div><span class="eyebrow">ALL RETAINED SCENES</span><h2>Deep-water glint-correction results</h2></div>
    <p>Select a date to inspect the offshore-water ROIs, correction stages, residual-glint decision and final products.</p>
  </div>
  <iframe title="Tidung deep-water results" src="Tidung_All_Scene_Results.html?v=20260914-fixed-rgb" style="display:block;width:100%;height:calc(100vh - 150px);min-height:900px;border:0;background:#fff"></iframe>
</section>
<section id="shallow-results" class="panel">
  <div class="section-head">
...
'''
    ...
    merged = merged.replace(
        ...
        "setTimeout(()=>{maps.forEach(m=>{m.invalidateSize();if(b.dataset.tab==='example')m.fitBounds([[-5.82526960465885,106.46469848255522],[-5.774789209564791,106.54013804997781]],{padding:[8,8]})});const f=document.querySelector('#deep-results iframe');if(f&&b.dataset.tab==='deep-results')f.contentWindow.postMessage('fit-tidung','*')},180)",
    )
...
<section id="results" class="panel"><div class="status" id="status"></div><div class="card"><h2>Original, first correction and final retained image</h2><div class="controls"><label><b>RGB display</b> <select id="rgbMode"><option value="water">Water-enhanced RGB — fixed range for all scenes</option><option value="natural">Natural RGB — range optimized for this scene</option></select></label></div><div id="rgbExplanation" class="note"></div><p class="chart-note">All three panels always use the same display range within the selected mode. Therefore differences between Original, Cycle 0 and Final are not caused by separate stretching. Click an image to open it large.</p><div class="maps"><div><div class="map-title">Original Sentinel-2 L2A</div><div id="map0" class="map"></div><button class="open-large" data-image="before">Open image large</button></div><div><div class="map-title">After Cycle 0</div><div id="map1" class="map"></div><button class="open-large" data-image="cycle0">Open image large</button></div><div><div class="map-title">Final retained product</div><div id="map2" class="map"></div><button class="open-large" data-image="final">Open image large</button></div></div></div><div id="decisionText" class="card note"></div></section>
...
<section id="products" class="panel"><div class="card"><h2>Final products</h2><div class="downloads" id="downloads"></div></div><div class="card"><h2>Rrs and rrs checks</h2><table id="qcTable"></table><p class="small"><b>Important:</b> Rrs_approx = final L2A BOA reflectance / π. B01, B02, B03 and B04 provide the four nominal inversion wavelengths 443, 492, 560 and 665 nm. B01 is aligned to the 20 m grid but retains native 60 m spatial information. Below-surface rrs_approx = Rrs_approx / (0.52 + 1.7 Rrs_approx). These conversions passed numeric checks, but they are not field-validated water-leaving reflectance.</p></div></section>
...
function selectedRgb(s){return document.getElementById('rgbMode').value==='water'?s.waterImages:s.images}
function rgbExplanation(){const mode=document.getElementById('rgbMode').value,r=DATA.fixedRgb;if(mode==='water'){document.getElementById('rgbExplanation').innerHTML=`<b>Water-enhanced RGB:</b> B04 is red (${r.low[0].toFixed(6)}–${r.high[0].toFixed(6)}), B03 is green (${r.low[1].toFixed(6)}–${r.high[1].toFixed(6)}), and B02 is blue (${r.low[2].toFixed(6)}–${r.high[2].toFixed(6)}). These fixed limits and gamma ${r.gamma.toFixed(2)} are identical for every retained date and stage. <b>How to read:</b> darker water has lower visible reflectance; brighter water has higher visible reflectance. Blue/cyan/green changes show the relative balance of the three bands, not a named water class. Pale or white areas are very bright or clipped. This is a visual aid, not the numerical glint test.`}else{document.getElementById('rgbExplanation').innerHTML='<b>Natural RGB:</b> B04 is red, B03 is green and B02 is blue. The range is calculated once from the three stages of the selected scene (1st–99.5th percentiles; gamma 0.85), so Original, Cycle 0 and Final remain directly comparable within that scene. Because another date can use a different range, use Water-enhanced RGB for comparisons between dates.'}}
function update(){const s=DATA.scenes[current],shown=selectedRgb(s);rgbExplanation();document.getElementById('status').innerHTML=metric('Scene',s.scene)+metric('Final product',s.retained)+metric('Additional correction',s.extra?'APPLIED':'NOT REQUIRED/AUTHORIZED')+metric('Visible negatives',s.qc.negative.toFixed(3)+'%')+metric('Downstream numeric QC',s.downstream);document.getElementById('decisionText').innerHTML=`<h3>Decision</h3>${s.reason}. ${s.extra?'This scene passed the residual-glint evidence, safety QC and final re-test.':'The first correction was retained because stronger subtraction was not scientifically supported.'}`;maps.forEach((m,i)=>{m.eachLayer(l=>{if(l instanceof L.ImageOverlay)m.removeLayer(l)});const overlay=L.imageOverlay(shown[['before','cycle0','final'][i]],s.bounds,{opacity:1,interactive:true}).addTo(m);overlay.on('click',()=>openLarge(['before','cycle0','final'][i]));m.fitBounds(s.bounds);m.setZoom(m.getZoom()+1)});document.getElementById('downloads').innerHTML=`<a class="button" target="_blank" href="${s.downloads.boa}">Final BOA folder</a><a class="button" target="_blank" href="${s.downloads.Rrs}">Rrs GeoTIFFs (B01–B8A)</a><a class="button" target="_blank" href="${s.downloads.rrs}">rrs GeoTIFFs (B01–B8A)</a><a class="button" target="_blank" href="PIXEL_RRS_EXPORT/XLSX/${s.scene}_pixel_Rrs_rrs.xlsx">Per-pixel Excel</a><a class="button" target="_blank" href="PIXEL_RRS_EXPORT/XLSX/Tidung_pixel_Rrs_rrs_index.xlsx">Excel index</a><a class="button" target="_blank" href="${s.downloads.qc}">Band QC CSV</a>`;document.getElementById('qcTable').innerHTML=`<tr><th>Check</th><th>Result</th><th>Meaning</th></tr><tr><td>Median Rrs B01 / B02 / B03 / B04</td><td>${s.qc.RrsB01.toFixed(5)} / ${s.qc.RrsB02.toFixed(5)} / ${s.qc.RrsB03.toFixed(5)} / ${s.qc.RrsB04.toFixed(5)} sr⁻¹</td><td>Four-band inversion input medians at nominal 443 / 492 / 560 / 665 nm</td></tr><tr><td>Maximum visible negative pixels</td><td>${s.qc.negative.toFixed(3)}%</td><td>Checks over-correction</td></tr><tr><td>Rrs ↔ rrs round-trip error</td><td>${s.qc.roundtrip.toExponential(2)}</td><td>Checks conversion arithmetic</td></tr><tr><td>Provisional downstream eligibility</td><td><b>${s.downstream}</b></td><td>Numeric readiness only; not field validation</td></tr>`;renderEvidence(s);drawChart()}
function openLarge(stage){const s=DATA.scenes[current],names={before:'Original Sentinel-2 L2A',cycle0:'After Cycle 0',final:'Final retained product'};document.getElementById('lightboxTitle').textContent=`${s.scene} — ${names[stage]}`;document.getElementById('lightboxImage').src=selectedRgb(s)[stage];document.getElementById('lightbox').showModal()}
